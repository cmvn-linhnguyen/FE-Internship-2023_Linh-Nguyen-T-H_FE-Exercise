...
export const renderTable = (cartItems) => {
    ...
    cartItems.forEach((item) => {
        ...
        <td class="col col-2">
          <div class="flex-row flex-center table-quantity-wrap">
            <button class="quantity-update decrease pro-${item.id}" ${item.quantity === 1 ? `disabled` : ``}>-</button>
    ...
    tableRows.forEach((row, index) => {
        ...
        const updateQuantity = (change) => {
            ...
            priceElement.textContent = `Total: $${cart.cartItems[index]
                ...
                .toFixed(2)}`;
            cart.saveCartItems();
            const decreaseButton = document.querySelector(`button.quantity-update.decrease.pro-${cart.cartItems[index].id}`);
...
export const updateCartQuantity = (cartQuantity) => {
...
export const displayCartTable = (cartItems) => {
    const cart = new Cart(cartItems);
    updateCartQuantity(cart.calcTotalProduct());
